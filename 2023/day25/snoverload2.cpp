...
auto bfs = [&](const auto &edges, string from,
               string to) {  // automatically deduce type (lambda function) [&]
  ...
  while (!paths.empty()) {
    auto path =
        move(paths.front());  // std library, move operation - ref to front
    ...
    for (auto edge_to : edges[path.back()]) {
      if (visited.contains(edge_to))
        continue  // instead of find

            visited.insert(edge_to);
      auto next = path;
      next.push_back(edge_to);
      paths.push(
          move(next));  // move is an optimizer.  still not sure when to use it.
    }
  }
  return {};
};
