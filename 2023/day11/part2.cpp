...
vector<string> expand_galaxy(vector<string> orig, int h, int w) {
  ...
  for (int r = 0; r < new_galaxy.size(); ++r) {
    ...
    for (int c = 0; c < row.size(); ++c) {
      ngalaxyr += row[c];
      if (find(empty_cols.begin(), empty_cols.end(), c) != empty_cols.end())
        ngalaxyr += 'm';
    }
    ...
    for (int j = i + 1; j < items.size(); ++j) {
      ...
      */
      ...
      for (int i = miny + 1; i <= maxy; ++i) {
        if (expanded[i][minx] == 'm') dist += 999998;

        dist++;
      }
      for (int j = minx + 1; j <= maxx; ++j) {
        if (expanded[maxy][j] == 'm') dist += 999998;
